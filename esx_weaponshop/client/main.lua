...
AddEventHandler('esx_weashop:clipcli', function(silahHash, sarjorTipi, mermiSayisi)
  ped = GetPlayerPed(-1)
  if IsPedArmed(ped, 4) then
    hash=silahHash
    if hash~=nil then
      TriggerServerEvent('esx_weashop:remove', sarjorTipi)
      AddAmmoToPed(GetPlayerPed(-1), hash, mermiSayisi)
      --ESX.ShowNotification("Şarjörü kullandın.")
	  exports.pNotify:SendNotification(
                            {
                                text = ("Şarjörü kullandın."),
                                type = "info",
                                timeout = 3000,
                                layout = "topRight",
								theme = "metroui"
      ...
      --ESX.ShowNotification("Elinde silah yok!")
	  exports.pNotify:SendNotification(
                            {
                                text = ("Elinde silah yok!"),
                                type = "error",
                                timeout = 3000,
                                layout = "topRight",
								theme = "metroui"
    ...
    --ESX.ShowNotification("Mermiler elindeki silaha uygun değil!")
	exports.pNotify:SendNotification(
                            {
                                text = ("Mermiler elindeki silaha uygun değil!"),
                                type = "error",
                                timeout = 3000,
                                layout = "topRight",
								theme = "metroui"
